...
def test_daily_reminder_force_resends_to_existing_recipient(settings, tmp_path):
    from electricity_app.db import Database
    ...
    now = datetime(2026, 7, 29, 23, 30, tzinfo=SHANGHAI)
    summary = Mock(spec=DashboardSummary)
    summary.is_stale = False
    summary.today_energy = Decimal("4.5")
    summary.today_cost = Decimal("2.7")
    ...
    assert service.send_today(now) == 1
    assert service.send_today(now, force=True) == 1
    assert client.send_template.call_count == 2
